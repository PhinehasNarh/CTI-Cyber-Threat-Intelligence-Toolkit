...
@router.get("")
async def dashboard_overview(db: AsyncSession = Depends(get_db), ws: int = Depends(active_workspace)):
    """Main dashboard stats — article counts, IOC counts, recent activity."""
    now = datetime.now(timezone.utc)
    today_start = now.replace(hour=0, minute=0, second=0, microsecond=0)

    # Article stats
    total_articles = await db.execute(select(func.count(FeedArticle.id)).where(FeedArticle.workspace_id == ws))
    articles_today = await db.execute(
        select(func.count(FeedArticle.id)).where(FeedArticle.workspace_id == ws, FeedArticle.fetched_at >= today_start)
    )
    unread_articles = await db.execute(
        select(func.count(FeedArticle.id)).where(FeedArticle.workspace_id == ws, FeedArticle.is_read == False)
    )

    # IOC stats
    total_iocs = await db.execute(select(func.count(IOC.id)).where(IOC.workspace_id == ws))
    iocs_today = await db.execute(
        select(func.count(IOC.id)).where(IOC.workspace_id == ws, IOC.first_seen >= today_start)
    )

    # Recent articles
    recent = await db.execute(
        select(FeedArticle)
        .where(FeedArticle.workspace_id == ws)
    ...
    for i in range(6, -1, -1):
        ...
        count_result = await db.execute(
            select(func.count(IOC.id)).where(
                IOC.workspace_id == ws, IOC.first_seen >= day_start, IOC.first_seen < day_end
            )
        )
        ioc_trend.append({
